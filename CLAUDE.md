...
- 会话持久化（法律 Web 链路已实现）：`agent_system/storage/` + `data/sessions/`；设计文档 `AGENT_SESSION_PERSISTENCE_PLAN.md`。
...
tests/                      unittest 测试
data/                       法条原始数据、Chroma 持久化目录和会话存档（data/sessions/）
```
...
注意：本项目已用 git 管理。真实 API key 不进 git：放环境变量，或放 `agent_system/config_local.py`（已被 `.gitignore` 排除，模板 `agent_system/config_local.example.py`），读取优先级为环境变量 > config_local > 内置默认值。`data/chroma/`、`data/sessions/`、`web_app/static/`、`node_modules/` 不入库。BGE-M3 首次运行可能下载模型；CUDA 不稳可切 CPU。
...
- `openai_client.py`：`OpenAIChatClient`，封装 Responses API。
...
- 子任务结果通过 `AgentEvent` 暴露，如 `legal_step`、`legal_rag_query_started`、`case_state_updated`、`legal_missing_details_suggested`、`legal_supplement_required`、`legal_case_rag_done`、`legal_web_search_started/done`、`legal_reference_materials`、`legal_next_action_decided`、`answer_delta`。
...
- `web_app/server.py`：FastAPI 服务入口，提供首页、健康检查、RAG 预热、NDJSON 聊天接口和历史会话 API（`GET /api/sessions`、`GET/DELETE /api/sessions/{id}`）。
...
设计文档：`AGENT_SESSION_PERSISTENCE_PLAN.md`

状态：法律咨询 Web 链路已实现（`agent_system/storage/session_store.py` + `web_app/server.py`）；CLI 和通用 AgentSession/ChatSession 尚未接入。
...
data/sessions/<session_id>/
  ...
  events.jsonl    session_created / turn_committed / turn_failed（legal_session_event.v1）
```
